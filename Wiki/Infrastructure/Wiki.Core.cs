...
namespace Wiki.Infrastructure.Core
{
	...
	public class UserNameAndPasswordAuthentication : IAuthentication, IBoundToAction
	{
		...
		public void Initialize(RouteInfo routeInfo)
		{
			...
			if (routeInfo.ActionParams.Length > 0)
			{
				var dc = routeInfo.ActionParams.FirstOrDefault(x => x.GetType().GetInterface("IData", true) != null) as IData;
				var upw = routeInfo.ActionParams.FirstOrDefault(x => x != null && x.GetType() == typeof(UnameAndPassword)) as UnameAndPassword;

				if (upw != null && dc != null)
				{
					WikiUser u = dc.GetUserByUserName(upw.UserName);

					if (u != null && u.Identifier == upw.Password)
					{
						Authenticated = true;
						Name = u.UserName;
						Identifier = "hidden";
